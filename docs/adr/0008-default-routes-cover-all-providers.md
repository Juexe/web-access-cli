...
省略 Capability 的 Route 时，默认 Route 包含支持该能力的全部内置 Instance；新增 Provider Type 时必须明确安排默认顺序。Search 顺序为 `tavily -> exa -> bocha -> brave -> searxng -> anysearch -> xcrawl -> deepseek`，Extract 顺序为 `firecrawl -> jina -> exa -> anysearch -> xcrawl -> http`，通用 HTTP 保持最终后备。DeepSeek 是完整模型轮次，放在 Search Route 末尾以避免不必要的延迟与费用；博查位于 Exa 之后、Brave 之前。
